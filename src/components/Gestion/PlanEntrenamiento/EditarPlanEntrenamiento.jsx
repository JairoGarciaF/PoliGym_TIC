...
export const EditarPlanEntrenamiento = ({ trainingPlan, onBack }) => {
    const [nombre, setNombre] = useState(trainingPlan?.nombre || '');
    const [imagenPlanEntrenamiento, setImagenPlanEntrenamiento] = useState(trainingPlan.imagenPlanEntrenamiento || null);
    const [descripcion, setDescripcion] = useState(trainingPlan?.descripcion || '');
    const [duracion, setDuracion] = useState(trainingPlan?.duracion || 0);
    const [dificultad, setDificultad] = useState(trainingPlan?.dificultad || '');
    const [detalleDias, setDetalleDias] = useState({

        lunes: trainingPlan?.detalleDias.lunes || "Descanso",
        martes: trainingPlan?.detalleDias.martes || "Descanso",
        miércoles: trainingPlan?.detalleDias.miércoles || "Descanso",
        jueves: trainingPlan?.detalleDias.jueves || "Descanso",
        viernes: trainingPlan?.detalleDias.viernes || "Descanso"
    });
