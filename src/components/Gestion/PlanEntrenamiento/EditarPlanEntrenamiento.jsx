...
import { updateTrainingPlan } from "../../../services/plans/trainingPlan";
...
export const EditarPlanEntrenamiento = ({ plan, onBack, refreshData }) => {
  const [trainingPlan, setTrainingPlan] = useState({
    name: plan?.name || "",
    level: plan?.level || "",
    description: plan?.description || "",
    startDate: dayjs(plan?.startDate) || null,
    endDate: dayjs(plan?.endDate) || null,
    workoutsIds: getIds(plan?.workouts) || [],
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateTrainingPlan(plan.id, trainingPlan);
      Swal.fire({
        title: "Plan de entrenamiento editado",
        text: "El plan de entrenamiento se ha editado correctamente",
        icon: "success",
        confirmButtonText: "Aceptar",
        confirmButtonColor: "#16243e",
      }).then(() => {
      ...
      console.error("Error al editar el plan de entrenamiento", error);
      Swal.fire({
        title: "Error",
        text: "Ha ocurrido un error al editar el plan de entrenamiento",
        icon: "error",
        confirmButtonText: "Aceptar",
        confirmButtonColor: "#16243e",
      });
